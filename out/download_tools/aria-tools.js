...
function getStatus(dlDetails, callback) {
    aria2.tellStatus(dlDetails.gid, ['status', 'totalLength', 'completedLength', 'downloadSpeed', 'files'], (err, res) => {
        if (err) {
...
function driveUploadFile(dlDetails, filePath, fileName, fileSize, callback) {
    drive.uploadRecursive(dlDetails, filePath, constants.GDRIVE_PARENT_DIR_ID, (err, url, isFolder) => {
